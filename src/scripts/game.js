...
export class Game {
  ...
  manualAction(){
    ...
    this.city.manualSimulate();

    this.allowAction = false;
    //window.ui.toggleActionButton(this.allowAction)
    window.ui.toggleBuildingButton(true);
    window.ui.updateStatusBar(this);

    //CHECK for GAME OVER
    let gameoverText = ''
    if(this.city.energy <= 0)
      gameoverText = 'You did not provide enough energy for the population'
    else if(this.city.food <= 0)
      gameoverText = 'You did not provide enough food for the population'
    else if(this.city.ghg >= 50)
      gameoverText = 'The city is over polluted'

    if(gameoverText != '')
    {
      const textElement = document.getElementById('game-over');
      if (textElement.style.display === 'none' ) {
        textElement.style.display = 'block'; // Show the element
      }

      textElement.innerHTML += '<br><span class="small-text"> '+ gameoverText+ '</span>'
    }
  ...
  useTool() {
    switch (window.ui.activeToolId) {
      ...
      default:
        ...
        if(window.ui.activeToolId == 'residential')
          deductedMoney = -200;
        else if (window.ui.activeToolId == 'industrial')
          deductedMoney = -400;

        let enoughMoney = this.city.money + deductedMoney;

        if (enoughMoney>=0 && !this.allowAction && this.focusedObject) { // 
          const { x, y } = this.focusedObject;
          this.city.placeBuilding(x, y, window.ui.activeToolId);
          this.city.occupiedTilesSet.add(`${x},${y}`);
          
          //update buildingcity list
          this.city.buildingList.push(window.ui.activeToolId);

          this.allowAction = true; //to prevent multi clicks
          //window.ui.toggleActionButton(this.allowAction);
          window.ui.updateMoney(this, deductedMoney)
          //only allow building once
          window.ui.toggleBuildingButton(false)//.toggleActionButton(this.allowAction);
        }
        break;
    }
  }
